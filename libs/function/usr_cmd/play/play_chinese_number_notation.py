...
@channel.use(
    ListenerSchema(
        ...
        inline_dispatchers=[
            Twilight([FullMatch("播放"), "anything" @ WildcardMatch()])
        ],
    )
)
async def main(
    ...
    try:
        Permission.group_permission_check(
            group, "play_chinese_number_notation"
        )
    ...
    try:
        ...
        await app.send_group_message(group, MessageChain(f"播放：不配：{e}"))
    ...
    if anything.matched:
        ...
        if speed > 1000 or speed < 1:
            await app.send_group_message(
                group, MessageChain("Bad Speed")
            )
        else:
            logger.info("Now Generating Sound...")
            audio_file_path = await asyncio.to_thread(
                number_notation_to_silk, speed, score_info[1]
            )
            if audio_file_path:
                logger.info(
                    "Generating Sound Successful, file path:"
                    f" {audio_file_path}"
                )
            await app.send_group_message(
                group, MessageChain([Voice(path=audio_file_path)])
            )
...
def match_note(note_list):
    ...
    if note_list[2] == ".":
        return NOTE_ORDER[note_to_match] + 1
    elif note_list[2] == "`":
        return NOTE_ORDER[note_to_match] + 25
    elif note_list[2] == "``":
        return NOTE_ORDER[note_to_match] + 37
    else:
        return NOTE_ORDER[note_to_match] + 13
...
def number_notation_to_silk(speed, score):
    ...
    for i in score:
        if i == " ":
            ...
            if re.match("[b#]", i):  # is a sign
                ...
                re.match("[\d]", i) and next_is_note
            ):  # is a number, and the previous char is a sign
                num_array[-1] += i
                next_is_note = False
            elif re.match(
                "[`.]", i
            ):  # ` and . have to come with a note
                num_array[-1] += i
            else:
                num_array.append(i)  # Start a new element
    ...
    for chords in list_to_change:
        ...
        x = np.linspace(
            0,
            time_const * chord_length,
            num=int(time_const * framerate * chord_length),
        )
        y_sum = x
        for note, length in chords:
            y = np.sin(2 * np.pi * FREQUENCY[note] * x) * volume
            if y.size > 1000:
                fade_in_out(y)
            else:
                fade_len = y.size // 2
    ...
    file_path = f"data/play/{req_id}.silk"
    wf = wave.open(f"data/play/{req_id}.wav", "wb")
    wf.setnchannels(1)
    ...
    silkcoder.encode(
        f"data/play/{req_id}.wav",
        f"data/play/{req_id}.silk",
        rate=44100,
        ios_adaptive=True,
    )
    os.remove(f"data/play/{req_id}.wav")
    return file_path
