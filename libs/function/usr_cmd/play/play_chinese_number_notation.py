...
async def main(app: Ariadne, member: Member, group: Group, anything: RegexResult):
    ...
    if anything.matched:
        ...
        if speed > 1000 or speed < 1:
            ...
            logger.info(f"Now Generating Sound...")
            audio_file_path = await asyncio.to_thread(number_notation_to_silk, speed, score_info[1])
            if audio_file_path: logger.info(f"Generating Sound Successful, file path: {audio_file_path}")
            await app.send_group_message(
                group,
...
def number_notation_to_silk(speed, score):
    ...
    silkcoder.encode(f'data/play/{req_id}.wav', f'data/play/{req_id}.silk', rate=44100, ios_adaptive=True)
