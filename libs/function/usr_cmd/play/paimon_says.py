...
async def main(
    ...
    try:
        ...
        await app.send_group_message(group, MessageChain(f"说：不配：{e}"))
    ...
    if speaker == "派蒙":
        ...
        logger.info(
            f"Paimon VITS: {speaker}, {req_content}, length ="
            f" {len(req_content)}"
        )
        ...
        logger.info(
            f"Genshin VITS: {speaker}, {req_content}, length ="
            f" {len(req_content)}"
        )
    elif speaker == "宁宁":
        audio_file_path = await asyncio.to_thread(
        ...
        logger.info(
            f"MoeGoe VITS: {nene_speaker_list[0]}, {req_content},"
            f" length = {len(req_content)}"
        )
        ...
        logger.info(
            f"MoeGoe VITS: {speaker}, {req_content}, length ="
            f" {len(req_content)}"
        )
    else:
        audio_file_path = await asyncio.to_thread(
