...
from libs.helper.backpack import grant_player_item, get_backpack_brief, remove_player_item
...
async def cmd_find_p(app: Ariadne, member: Member, group: Group):
    ...
    if len(backpack_brief) > 0:
        await app.send_group_message(
            group,
            MessageChain(f"你背包内有\n{backpack_brief}")
        )
    else:
        await app.send_group_message(
            group,
            MessageChain("你背包内么的东西啊")
        )
