...
async def weather_CN(
    ...
    if not anything.matched or anything.result is None:
        ...
        if weather_data is not None:
            await app.send_group_message(group, weather_data)
        else:
            await app.send_group_message(
                group, MessageChain(f"找不到城市：{city}")
            )
...
async def weather_global(
    ...
    if not anything.matched or anything.result is None:
        ...
        city = anything.result.display.strip()
        logger.info(f"查查 {city}")
        city, local_names, weather_data = await asyncio.to_thread(
        ...
        if weather_data is None:
            ...
            await app.send_group_message(
                group, MessageChain(ImageElement(data_bytes=imgByteArr))
            )
...
def render_open_weather_data(
    city_name: str, local_names: dict, weather_data: dict
):
    text_box = (400, 230)
    ...
    (x, y) = draw.textsize(
        weather_data["weather"][0]["description"], font=font_title
    )
    ...
    if "ja" in local_names.keys():
        weather_city_name = (
            f"{weather_data['name']}, {local_names['ja']}"
        )
    else:
        weather_city_name = f"{weather_data['name']}"
    draw.text(
        pointer, weather_city_name, font=font_text, fill=(255, 255, 255)
    )

    pointer = (10, pointer[1] + 34)
    weather_real_time = f"实时温度: {weather_data['main']['temp']}°C"
    draw.text(
        pointer, weather_real_time, font=font_text, fill=(255, 255, 255)
    )
    ...
    draw.text(
        pointer, weather_humidity, font=font_text, fill=(255, 255, 255)
    )
...
def mj_router(type_search: TypeMJSearch):
    ...
    match type_search:
        case TypeMJSearch.CAR_LIM:
            return mj_url + "limit"
        case TypeMJSearch.AQI:
            return mj_url + "aqi"
        case TypeMJSearch.LIFE_INDEX:
            return mj_url + "index"
        case TypeMJSearch.WARNING:
            return mj_url + "alert"
        case TypeMJSearch.WEATHER_FORECAST_24H:
            return mj_url + "forecast24hours"
        case TypeMJSearch.WEATHER_FORECAST_15D:
            return mj_url + "forecast15days"
        case TypeMJSearch.WEATHER_REAL_TIME:
            return mj_url + "condition"
        case TypeMJSearch.AQI_FORECAST_5D:
            return mj_url + "aqiforecast5days"
...
def get_weather_info(city: str):
    ...
    if id_list.value_counts == 0:
        ...
        with session.post(
            mj_router(TypeMJSearch.WEATHER_REAL_TIME),
            headers=headers,
            data=real_time_param,
        ) as res:
            try:
