...
@channel.use(
    ListenerSchema(
        ...
        inline_dispatchers=[Twilight([FullMatch("直播监控"), "anything" @ WildcardMatch()])]
    )
)
async def main(app: Ariadne, member: Member, group: Group, anything: RegexResult):
    ...
    # Permission
    try:
        Permission.group_permission_check(group, "bilibili_live_monitor")
    ...
    if not anything.matched or anything.result is None:
        ...
        if cmd.startswith("添加"):
            # 添加频道
            m = re.match('添加 +((https://)?live\.bilibili\.com/)?(.+)', cmd)
            if m is not None:
                room_id = m[3]
                add_subscription_result = await add_subscription_room_for_group(room_id, group)
                if add_subscription_result == 1:
                    await app.send_group_message(
                        group,
                        MessageChain(f"添加频道成功")
                    )
                elif add_subscription_result == 2:
                    await app.send_group_message(
                        group,
                        MessageChain(f"找不到频道")
                    )
                elif add_subscription_result == 3:
                    await app.send_group_message(
                        group,
                        MessageChain(f"该频道已经有啦")
                    )
            else:
                await app.send_group_message(
                    group,
    ...
    for room_id, info in livedata.items():
        if group.id in info['subscribed_group']:
            if info['live_status'] == 1:
                subscribed_live_status += f"{room_id}: {info['owner']} 🔴ON AIR\n"
            elif info['live_status'] == 2:
                subscribed_live_status += f"{room_id}: {info['owner']} 🈲封禁中\n"
            else:
                subscribed_live_status += f"{room_id}: {info['owner']}\n"

    return subscribed_live_status[:-1]

async def add_subscription_room_for_group(room_id: str, group: Group) -> int:
    # 添加频道
    # 0: unknown error
    # 1: success
    # 2: no such room
...
async def update_bilibili_live_status():
    ...
    for room_id in livedata.keys():
        ...
        if livedata[room_id]['live_status'] == 0 and info['room_info']['live_status'] == 1:
            # now on air
            livedata[room_id]['live_status'] = 1
            now_on_air[room_id] = livedata[room_id]
        elif livedata[room_id]['live_status'] == 1 and info['room_info']['live_status'] == 0:
            livedata[room_id]['live_status'] = 0
        elif livedata[room_id]['live_status'] != info['room_info']['live_status']:
            livedata[room_id]['live_status'] = info['room_info']['live_status']
