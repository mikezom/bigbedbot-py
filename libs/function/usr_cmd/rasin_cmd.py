...
async def cmd_get_rasin(app: Ariadne, member: Member, group: Group):
    ...
    if res == -1:
        ...
        await app.send_group_message(
            group,
            MessageChain(f"你体力变成了负数，快艾特zom解决吧！")
        )
    else:
        await app.send_group_message(
            group,
            MessageChain(f"你现在有 {res}/160 体力。")
        )
...
@channel.use(
    ListenerSchema(
        listening_events=[GroupMessage],
        inline_dispatchers=[Twilight([FullMatch("给我体力"), "anything" @ WildcardMatch()])]
    )
)
async def cmd_debug_change_rasin(app: Ariadne, member: Member, group: Group, anything: RegexResult):
    try:
    ...
    await app.send_group_message(
        group,
        MessageChain(f"你现在有 {get_rasin(member.id)}/160 体力。")
    )