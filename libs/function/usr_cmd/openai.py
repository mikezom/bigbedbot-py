...
async def main(app: Ariadne, member: Member, group: Group, anything: RegexResult, message: GroupMessage):
    ...
    if not anything.matched or anything.result is None:
        ...
        if is_end_of_chat(prompt_msg):
            ...
            await app.send_group_message(
                group,
                MessageChain(f"End of This Conversation")
            )
        else:
            message_bundle = []
            ...
            if chat_history is None or chat_history == {}:
                clear_chat_history(member.id, HISTORY_PATH)
                message_bundle = [{"role": "user", "content": prompt_msg}]
            else:
                history_time, history_chat = float(chat_history["time"]), chat_history["chat"]
                if is_timeout(history_time) or history_chat is None or len(history_chat) == 0:
                    clear_chat_history(member.id, HISTORY_PATH)
                    message_bundle = [{"role": "user", "content": prompt_msg}]
                else:
                    message_bundle = chat_history["chat"]
                    message_bundle.append({"role": "user", "content": prompt_msg})

            logger.info(f"问问 {message_bundle}")

            gpt_msg_ret = await asyncio.to_thread(ask_chatGPT, message_bundle)

            if gpt_msg_ret is not None:
                message_bundle.append({"role": "assistant", "content": gpt_msg_ret})
                save_chat_history(member.id, HISTORY_PATH, message_bundle)
                await app.send_group_message(
                    group,
                    MessageChain(gpt_msg_ret),
                    quote=message.id
                )
            else:
                await app.send_group_message(
                    group,
                    MessageChain(f"GPT doesn't understand your question.")
                )
...
def save_chat_history(qq_id: int, history_path: str, chat_history):
    ...
    openai_chat_history[str(qq_id)] = {"time": time.time(), "chat": chat_history}
